...
> 异常时重启 AstrBot，再确认插件版本为 `0.2.0-beta`。
...
| 旧版命令 | 新版命令 | 含义 |
| --- | --- | --- |
| `!grouplist` | 原命令可直接使用 | 查看已开启学习、回复、允许自主管理、使用全局词库、仅使用本群词库及来源排除群；联动部分只显示 `!sharelist` 引导 |
| `!sharelist` | 新版只读命令 | 管理员私聊查看完整联动群聊列表，包括所有联动组名和成员群号；群聊中不会展开跨群信息 |
...
| `!add share <群号...> <联动组名>` / `!remove share <群号...> <联动组名>` | 新版联动命令 | 创建命名联动组或调整成员；成员群可查询组内所有成员的本群词库，不继承全局、外部词库或其他联动组权限 |
| `!add wellcome <欢迎语> <联动组名>` / `!remove wellcome <联动组名>` | 联动组欢迎命令 | 为已有联动组设置或关闭新成员欢迎语；首位新人入群后收集 60 秒，将期间新人合并为一条 `@成员... + 自定义文本` 消息，展示 60 秒后撤回；同时兼容 `welcome` 拼写 |
| `!add reply cd <分钟> <联动组名>` / `!remove reply cd <联动组名>` | 联动组回复冷却 | 为联动组成员群统一配置独立冷却时长；某个群成功回复后只冷却该群，不影响同组其他群或欢迎语 |
| `!add sanhao <联动组名>` / `!remove sanhao <联动组名>` | 三好学习模式 | 开启后，成员群遇到含图片的问题时只使用含图片的答案；纯文本及其他无图片答案会被跳过，同时兼容 `threegood` 别名 |
| `!d reply <完整答案正文>` | 跨群答案删除 | 在全部群聊自产词库和外部导入词库中删除完全一致的纯文本答案；执行前自动备份，群聊回执不显示来源群号 |
...
| `!reply -s <类型> <百分比> <群号...>` | 新版消息类型概率命令 | 只设置目标群指定触发消息类型的回复概率；支持 `text`、`image`、`face`、`marketface`、`xml`、`json`、`record`、`video`、`file`、`forward`、`share`、`music`、`dice`、`mixed`、`other` |
| `!reply -d <类型> <群号...>` | 新版消息类型概率命令 | 删除指定类型的独立概率，恢复继承目标群基础概率 |

跨群写入使用与 WebUI 相同的配置 revision、持久化与审计服务；任一目标群或配置保存失败时不会保留部分修改。管理员可以在任意管理群或私聊 Bot 中发送带群号的跨群命令，不需要进入目标群。旧式 `!reply -s <概率> <群号...>` 会清除目标群已有的消息类型细分并统一全部类型；带类型的形式只修改对应类型。联动组不存在时由 `!add share` 自动创建，移除最后一个成员时自动删除空组；组名包含空格时需要使用引号。欢迎语只能绑定已有联动组，最大 1000 字，也可在 WebUI 群聊设置页管理。首位新人会启动固定 60 秒聚合窗口，同一成员在窗口内只 `@` 一次；合并欢迎消息发送后展示 60 秒并自动撤回。联动组回复冷却范围为 1 至 10080 分钟；设置作用于组内每个成员群，但各群分别记录最近回复时间，某群回复不会冷却同组其他群。一个群属于多个已设置冷却的联动组时，对该群采用最长冷却时长。冷却计时与现有普通群冷却一致，仅保存在运行内存中，插件重载后重新开始；欢迎消息不受影响。`!grouplist` 不展示联动组名或成员群号，只提供 `!sharelist` 引导；完整联动群聊列表仅在管理员私聊使用 `!sharelist` 时返回。其他群聊回执也只显示操作结果和当前群状态。`!add subadmin` 需要 Bot 能通过 NapCat 读取目标群资料，并保存执行时的群主和群管理员 QQ 号；群管理成员发生变化后可再次执行该命令刷新授权名单。
...
- 可关闭的 `!learning`、`!reply`，以及 `!grouplist`、`!sharelist`、`!add/!remove learning|learnings|reply|tag|subadmin|unmerge|globe|share` 跨群管理入口
...
- 按群、按触发消息类型独立设置回复概率；旧概率命令可统一重置全部类型
- 问题与答案内容一致时标记为复读词库，每群滚动一小时最多成功触发两次
...
- 命名群聊联动词库只联合成员群的本群词库，不递归传播其他词库权限
- 联动组可配置仅在成员群生效的新人欢迎消息；60 秒内加入的成员合并 `@` 后发送，展示 60 秒自动撤回，并由命令、WebUI 与 AstrBot 配置共享同一配置源
- 联动组可为成员群统一配置独立回复冷却时长；各群分别计时，互不影响，也不影响新人欢迎消息
- 联动组可开启三好学习模式；含图片的问题只从含图片的答案中选择，学习采集、文本问题、欢迎语和回复冷却保持原行为
- AstrBot 全局管理员、插件管理员与按群子管理员权限
- 管理员引用词库回复执行 `!d` / `!delete`，或用 `!d <序号>` 删除最近回复
- 全局/插件管理员可用 `!d reply <完整答案正文>` 跨群删除所有完全一致的纯文本答案；近似文本、包含匹配和媒体混合答案不会被误删
