...
| `!add reply cd <分钟> <联动组名>` / `!remove reply cd <联动组名>` | 联动组回复冷却 | 设置或关闭联动组共享回复冷却；任一成员群成功发送词库回复后，组内全部成员群在冷却期内跳过回复，欢迎语不受影响 |
...
- 联动组可配置共享回复冷却；任一成员群成功回复后，组内全部成员群在指定分钟内跳过词库回复，且不影响新人欢迎消息
