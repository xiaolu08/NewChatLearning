...
| `!add wellcome <欢迎语> <联动组名>` / `!remove wellcome <联动组名>` | 联动组欢迎命令 | 为已有联动组设置或关闭新成员欢迎语；新成员加入任一成员群时，Bot 会先 `@` 新成员再发送自定义文本；同时兼容 `welcome` 拼写 |
...
跨群写入使用与 WebUI 相同的配置 revision、持久化与审计服务；任一目标群或配置保存失败时不会保留部分修改。管理员可以在任意管理群或私聊 Bot 中发送带群号的跨群命令，不需要进入目标群。旧式 `!reply -s <概率> <群号...>` 会清除目标群已有的消息类型细分并统一全部类型；带类型的形式只修改对应类型。联动组不存在时由 `!add share` 自动创建，移除最后一个成员时自动删除空组；组名包含空格时需要使用引号。欢迎语只能绑定已有联动组，最大 1000 字，也可在 WebUI 群聊设置页管理。`!grouplist` 不展示联动组名或成员群号，只提供 `!sharelist` 引导；完整联动群聊列表仅在管理员私聊使用 `!sharelist` 时返回。其他群聊回执也只显示操作结果和当前群状态。`!add subadmin` 需要 Bot 能通过 NapCat 读取目标群资料，并保存执行时的群主和群管理员 QQ 号；群管理成员发生变化后可再次执行该命令刷新授权名单。
...
- 联动组可配置仅在成员群生效的 `@新成员 + 自定义文本` 欢迎消息，并由命令、WebUI 与 AstrBot 配置共享同一配置源
