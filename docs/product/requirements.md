...
- 联动组成员群独立回复冷却：管理员可为已有联动组配置 1 至 10080 分钟冷却；设置应用于所有成员群，但每个群分别记录最近成功回复时间，互不影响。欢迎语不检查也不刷新回复冷却。
- 跨群答案删除：全局管理员或插件管理员可按完整纯文本答案一次删除全部群聊自产词库和外部导入词库中的精确副本。执行前必须创建数据库备份，事务内同步失效回复追踪并清理空问题；群聊回执不得展示来源群号。
- 联动组三好学习模式：管理员可按联动组启停。开启后，成员群收到包含图片组件的问题时，只允许选择包含图片组件的答案；无图片候选时保持静默。模式不得改变学习采集内容，也不得影响普通文本问题、欢迎语或回复冷却。
...
| 2026-08-16 | 1.2 | 增加联动组共享回复冷却及欢迎语隔离要求。 |
| 2026-08-16 | 1.3 | 明确联动组冷却按成员群分别计时，不在组内跨群传播。 |
| 2026-08-16 | 1.4 | 增加跨群精确答案删除、备份、权限和隐私要求。 |
| 2026-08-16 | 1.5 | 增加联动组三好学习模式及图片问题回复约束。 |
